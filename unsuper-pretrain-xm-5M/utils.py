...
def load_knn(path):
	knn_result = []
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
		file = path + 'KNN_' + modal + '.npy'
		knn = np.load(file)
		knn_result.append(knn.astype(int))
	return knn_result
...
def load_all_query_url(feature_dir,list_dir, label_dim):
	...
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
		...
		test_feature.append(feature_list)
	...
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
		...
		database_feature.append(feature_list)
	...
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
		...
		test_label.append(feature_list)
	...
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
		...
		database_label.append(feature_list)
...
def load_train_feature(feature_dir):
	train_feature = []
	for modal in ['img', 'txt', 'aud', 'vid', '3d']:
